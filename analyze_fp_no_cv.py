#!/usr/bin/env python3
"""
Analysis script for FP1/FP2 VIGNet experiments (full frequency features)
...
def plot_rmse_comparison(df, output_path):
    ...
    ax.set_title('Test RMSE Comparison Across All Trials', fontsize=14, fontweight='bold')
...
def plot_correlation_comparison(df, output_path):
    ...
    ax.set_title('Test Correlation Comparison Across All Trials', fontsize=14, fontweight='bold')
    ax.set_xticks(trials)
    ax.set_ylim([0.7, 1.0])
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def plot_metrics_comparison(df, output_path):
    ...
    ax1.set_title('Test RMSE', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('Test Correlation', fontsize=14, fontweight='bold')
...
def plot_metrics_boxplot(df, output_path):
    ...
    ax1.set_ylabel('Test RMSE', fontsize=12)
    ax1.set_title('Test RMSE Distribution', fontsize=14, fontweight='bold')
    ...
    ax2.boxplot(df['Test_Correlation'].values, vert=True, patch_artist=True,
                ...
                medianprops=dict(color='red', linewidth=2))
    ax2.set_ylabel('Test Correlation', fontsize=12)
    ax2.set_title('Test Correlation Distribution', fontsize=14, fontweight='bold')
    ax2.set_ylim([0.7, 1.0])
...
def plot_scatter_all_trials(predictions, output_path):
    ...
    ax.set_title('Predicted vs True PERCLOS (All Trials, Test Set)', fontsize=14, fontweight='bold')
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def plot_time_series_sample(predictions, output_path, num_trials=2):
    ...
    for idx, trial in enumerate(selected_trials[:num_trials]):
        if 'test' in predictions[trial]:
            ...
            axes[idx].set_title(f'Trial {trial} - Time Series Comparison (Corr: {correlation:.4f}, RMSE: {rmse:.4f})',
...
def main():
    parser = argparse.ArgumentParser(description='Analyze FP1/FP2 VIGNet experiment results')
    parser.add_argument('--log-dir', type=str, default='./logs_fp_no_cv',
                       help='Directory containing log files and predictions')
    parser.add_argument('--output-dir', type=str, default='./analysis_results/fp_no_cv',
    ...
    print("="*80)
    print("FP1/FP2 VIGNet Analysis (Full Frequency Features)")
    print("="*80)
