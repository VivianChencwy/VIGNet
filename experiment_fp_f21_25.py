...
import network_fp_f21_25 as network
...
class experiment_fp_f21_25():
    ...
    """
    def __init__(self, trial_idx, gpu_idx, task, logger=None, log_dir="./logs_fp_f21_25"):
        # Assign GPU
        ...
        if logger is not None:
            ...
            self.logger = logging.getLogger(f"experiment_fp_f21_25_trial{trial_idx}")
        ...
        self.logger.info(f"START TRAINING - Task: {task} (FP1/FP2 only, de_LDS_f21-25, no CV)")
        self.logger.info(f"Features: de_LDS_f21-25 (41Hz, 43Hz, 45Hz, 47Hz, 49Hz)")
    ...
    def training(self):
        # Load dataset (FP1/FP2 only, f21-25 features, no CV)
        self.logger.info("Loading dataset (FP1/FP2 channels, de_LDS_f21-25 only, fixed split)...")
        ...
        # Call model (FP1/FP2 version, adapted for 5 frequency features)
        self.logger.info("Initializing VIGNet-FP model (2 channels, 5 frequency features)...")
        VIGNet = network.vignet_fp_f21_25(mode=self.task)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='VIGNet Training Script (FP1/FP2 only, de_LDS_f21-25, no CV)')
    ...
    parser.add_argument('--log-dir', type=str, default='./logs_fp_f21_25',
                       help='Directory for log files. Default: ./logs_fp_f21_25')
    ...
    summary_logger.info("SEED-VIG Training Summary (FP1/FP2 channels only, de_LDS_f21-25, no CV)")
    summary_logger.info(f"Start time: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    summary_logger.info("Features: de_LDS_f21-25 (41Hz, 43Hz, 45Hz, 47Hz, 49Hz)")
    ...
    for trial in trials_to_run:
        ...
        trial_logger.info(f"TRIAL {trial} - Task: {task} (FP1/FP2 only, de_LDS_f21-25, no CV)")
        ...
        main = experiment_fp_f21_25(trial_idx=trial, gpu_idx=0, task=task, logger=trial_logger, log_dir=main_log_dir)
        try:
